...
import { ok, serverError, badRequest } from '../../helppers/http-helper'

const makeEmailValidator = (): EmailValidator => {
  class EamilValidatorStub implements EmailValidator {
    isValid (email: string): boolean {
      return true
    }
  }
  return new EamilValidatorStub()
}
...
describe('SignUp Controller', () => {
  ...
  test('Shold return 400 if no password confirmation fails', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Shold call AddAccount whith correct values', async () => {
  ...
  test('Shold return 200 valid data is provided', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest())
